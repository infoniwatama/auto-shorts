...
【タイトル候補のフォーマット（バズ最適化、絶対遵守）】

# 絶対ルール
1. 各案は **28文字以内** （スマホ表示の打ち切り対策、ハッシュタグ含めない）
2. 最重要キーワード（数字 or 固有名詞）を必ず **冒頭13文字以内に置く**
3. 絵文字は **冒頭1個のみ** 、意味のあるもの（🚨⚡💥📊🔥）から選ぶ
4. ブランド表記（｜速報 等）は28文字に余裕がある時のみ。基本は外す
5. 数字は **具体数字** （1600万、3兆円）で。概数（数百万）禁止
6. 「衝撃」「驚愕」「バズ」だけのフックワードは禁止（検索ヒット0）

# 5案の型を必ず分散（同じ型を2本以上作らない）
- 案1【数字インパクト型】「[衝撃絵文字][具体数字][固有名詞][動詞]」
  例: 「🚨1600万件AI盗用、米3社が緊急同盟」(22字)
- 案2【対立構造型】「[A]vs[B]、[結論]」「[A]に[B]が宣戦布告」
  例: 「中国AIに米が宣戦布告、史上初3社連合」(20字)
- 案3【結論先出し型】「[最も重要な結論][文脈1単語]」
  例: 「AI業界の力学が一夜で変わった理由」(17字)
- 案4【疑問・カリギュラ型】「なぜ[主語]は[行動]？」
  例: 「なぜAI3社は突然手を組んだ？」(15字)
- 案5【損失回避・煽り型】「[負のキーワード]、[対象]に何が起きる」
