...
{{
  "title_candidates": ["案1(固有名詞詰め込み型)", "案2(数字インパクト型)", "案3(結論先出し型)", "案4(疑問・煽り型)", "案5(文脈追加型)"],
...
【タイトル候補のフォーマット（SEO最適化、重要）】
- **主要固有名詞・数字を最初の20文字以内に詰め込む**（YouTube検索で当たるため）
- 全体 25-28文字以内（モバイル表示で切れない、ハッシュタグ含めない）
- 絵文字は最大1個、末尾または【】内
- 「衝撃」「驚愕」「バズ」だけのフックワードは絶対NG（検索ヒット0）
- 必ず固有名詞（企業名・人名・国名）を **2つ以上** 入れる
- 数字（金額・人数・時期）が入れやすい場合は必ず入れる

【タイトル候補は5本、それぞれ違う切り口で】
1. 固有名詞詰め込み型: 「OpenAI Anthropic Google同盟、中国AI対策｜2026速報」
2. 数字インパクト型: 「1600万件AI盗用、米3社が緊急同盟｜2026速報」
3. 結論先出し型: 「中国AIに米が宣戦布告、史上初3社連合｜速報」
4. 疑問・煽り型: 「なぜAI3社は手を組んだ？｜2026最新解説」
5. 文脈追加型: 「Anthropic CEO発表、AI業界の力学が変わる｜速報」

【良い例】
- 「OpenAI Anthropic Google同盟成立、対中AI戦略｜2026速報」 ← 固有名詞3つ
- 「マスク9兆円Cursor買収、xAI統合へ｜2026最新」 ← 人名+数字+企業名
- 「Apple CEO Tim Cook退任、Ternus就任｜2026」 ← 企業+人名2つ

【悪い例（検索されない）】
- 「【🚨衝撃】AI業界に激震｜2026」 ← 固有名詞ゼロ
- 「ついに、あの企業が…｜2026」 ← キーワードなし
- 「業界騒然！3社の決断とは｜2026」 ← 中身ゼロ
